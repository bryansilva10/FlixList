...
export class UserEditComponent implements OnInit {
      ...
      .subscribe(
        response => {
          ...
          if (!response.user) {
            ...
            //update name in DOM after user updates it
            ...
            if (!this.filesToUpload) {
              ...
              this.makeFileRequest(this.url + 'upload-image-user/' + this.user._id, [], this.filesToUpload)
                .then(
                  (result: any) => {
                    //set user class property with what comes from request
                    this.user.image = result.image;
                    //modify user in local storage because it now has an image
                    localStorage.setItem('identity', JSON.stringify(this.user));
  ...
  makeFileRequest(url: string, params: Array<string>, files: Array<File>) {
    ...
    //return promise with request attempt
