...
import { GLOBAL } from './global'; //for api url
...
export class UserService {
  ...
  signup(user, getHash = null): Observable<any> {
    ...
    if (getHash != null) {
      //add get hash property to user object, before converting object to string...and store whatever was passed in getHash
      user.getHash = getHash;
    }

    //stringify user object and store as params
    let params = JSON.stringify(user);

    //define headers
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });

    //return the request to use observable
